...
######################################################################
#
# Char::OldUTF8 - Source code filter to old UTF-8 script
#
...
sub Char::OldUTF8::escape_script {
    ...
    if (/^ use Char::Eoldutf8(?:\s+[0-9\.]*)?\s*; $/oxms) {
        ...
        $e_script .= sprintf("use Char::Eoldutf8 %s;\n%s", $Char::Eoldutf8::VERSION, $use_re_eval); # require run-time routines version
...
Char::OldUTF8 - Source code filter to escape old UTF-8

=head1 SYNOPSIS

  use Char::OldUTF8;
  use Char::OldUTF8 version;         --- require version
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  m?regexp?               m?ditto@Char::Eoldutf8::matched?
  s/regexp/replacement/   ($_ =~ m/ditto@Char::Eoldutf8::matched/) ?
                          eval{ Char::Eoldutf8::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  m?regexp?b              m?regexp@Char::Eoldutf8::matched?
  s/regexp/replacement/b  ($_ =~ m/(\G[\x00-\xFF]*?)(?:regexp)@Char::Eoldutf8::matched/) ?
                          eval{ Char::Eoldutf8::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
