...
######################################################################
#
# Char::OldUTF8 - Source code filter to escape old UTF-8 script
#
# http://search.cpan.org/dist/Char-Char::OldUTF8/
#
...
BEGIN { CORE::require Char::Eoldutf8; }
...
sub unimport {}
sub Char::OldUTF8::escape_script;

# regexp of character
my $your_char = q{(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[\x00-\x7F\xF5-\xFF]};
...
my $qq_substr  = qr{(?: Char::OldUTF8::substr | CORE::substr | substr ) \( $qq_paren \)
                 }xms;
...
my $function_ord;         # ord()   to ord()   or Char::OldUTF8::ord()
my $function_ord_;        # ord     to ord     or Char::OldUTF8::ord_
my $function_reverse;     # reverse to reverse or Char::OldUTF8::reverse
my $function_getc;        # getc    to getc    or Char::OldUTF8::getc
...
perl $0 old UTF-8_script.pl > Escaped_script.pl.e
END
    }

    print Char::OldUTF8::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        ...
        if (eval q{ use Fcntl qw(O_WRONLY O_APPEND O_CREAT); 1 } and CORE::sysopen($fh,"$filename.e",&O_WRONLY|&O_APPEND|&O_CREAT)) {
            ...
            Char::Eoldutf8::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e";
        }
        ...
        my $e_script = Char::OldUTF8::escape_script($filename);
        print {$fh} $e_script;
    ...
    my $fh = gensym();
    Char::Eoldutf8::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e";
...
sub Char::OldUTF8::escape_script {
    ...
    # read old UTF-8 script
    my $fh = gensym();
    Char::Eoldutf8::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die __FILE__, ": Can't close file: $script";

    if (/^ use Char::Eoldutf8(?:\s+[0-9\.]*)?\s*; $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Char::Eoldutf8 %s;\n", $Char::OldUTF8::VERSION); # require run-time routines version

        # use Char::OldUTF8 version qw(ord reverse getc);
        $function_ord     = 'ord';
        $function_ord_    = 'ord';
        $function_reverse = 'reverse';
        $function_getc    = 'getc';
        if (s/^ \s* use \s+ Char::OldUTF8 \s* ([^\x80-\xFF;]*) ; \s* \n? $//oxms) {

            # require version
            my $list = $1;
            if ($list =~ s/\A ([0-9]+\.[0-9]+) \.0 \s* //oxms) {
                my $version = $1;
                if ($version ne $Char::OldUTF8::VERSION) {
                    my @file = grep -e, map {qq{$_/Char/OldUTF8.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Char/OldUTF8.pm $version, but @{[__FILE__]} is version $Char::OldUTF8::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Char/Eoldutf8.pm $version, but \\\$Char::Eoldutf8::VERSION is \$Char::Eoldutf8::VERSION" if \$Char::Eoldutf8::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ([0-9]+(?:\.[0-9]*)) \s* //oxms) {
                my $version = $1;
                if ($version > $Char::OldUTF8::VERSION) {
                    die "Script $0 required Char/OldUTF8.pm $version, but @{[__FILE__]} is only version $Char::OldUTF8::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Char::OldUTF8::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'Char::OldUTF8::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'Char::OldUTF8::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'Char::OldUTF8::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
# escape old UTF-8 part of script
sub escape {
...
# $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> Char::Eoldutf8::PREMATCH()
    elsif (/\G ( \$` | \$\{`\} | \$ \s* PREMATCH  \b | \$ \s* \{ \s* PREMATCH  \s* \} | \$ \s* \{\^PREMATCH\}  ) /oxmsgc) {
        $slash = 'div';
        return q{Char::Eoldutf8::PREMATCH()};
    }

# $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> Char::Eoldutf8::MATCH()
    elsif (/\G ( \$& | \$\{&\} | \$ \s* MATCH     \b | \$ \s* \{ \s* MATCH     \s* \} | \$ \s* \{\^MATCH\}     ) /oxmsgc) {
        $slash = 'div';
        return q{Char::Eoldutf8::MATCH()};
    }

# $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> Char::Eoldutf8::POSTMATCH()
    elsif (/\G (                 \$ \s* POSTMATCH \b | \$ \s* \{ \s* POSTMATCH \s* \} | \$ \s* \{\^POSTMATCH\} ) /oxmsgc) {
        $slash = 'div';
        return q{Char::Eoldutf8::POSTMATCH()};
    }
...
# while (<WILDCARD>) --- glob
    ...
    elsif (/\G \b while \s* \( \s* < ((?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF>\0\a\e\f\n\r\t])+?) > \s* \) \b /oxgc) {
        return 'while ($_ = Char::Eoldutf8::glob("' . $1 . '"))';
    }

# while (glob)
    elsif (/\G \b while \s* \( \s* glob \s* \) /oxgc) {
        return 'while ($_ = Char::Eoldutf8::glob_)';
    }

# while (glob(WILDCARD))
    elsif (/\G \b while \s* \( \s* glob \b /oxgc) {
        return 'while ($_ = Char::Eoldutf8::glob';
    }
...
# functions of package Char::Eoldutf8
    elsif (/\G \b (CORE:: | ->[ ]* (?: atan2 | [a-z]{2,})) \b  /oxgc) { $slash = 'm//'; return $1;                    }
    elsif (/\G \b bytes::substr \b (?! \s* => )                /oxgc) { $slash = 'm//'; return 'substr';              }
    elsif (/\G \b chop \b          (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::Eoldutf8::chop';         }
    elsif (/\G \b bytes::index \b  (?! \s* => )                /oxgc) { $slash = 'm//'; return 'index';               }
    elsif (/\G \b Char::OldUTF8::index \b   (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::OldUTF8::index';         }
    elsif (/\G \b index \b         (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::Eoldutf8::index';        }
    elsif (/\G \b bytes::rindex \b (?! \s* => )                /oxgc) { $slash = 'm//'; return 'rindex';              }
    elsif (/\G \b Char::OldUTF8::rindex \b  (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::OldUTF8::rindex';        }
    elsif (/\G \b rindex \b        (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::Eoldutf8::rindex';       }
    elsif (/\G \b lc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Eoldutf8::lc';           }
    elsif (/\G \b lcfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Eoldutf8::lcfirst';      }
    elsif (/\G \b uc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Eoldutf8::uc';           }
    elsif (/\G \b ucfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Eoldutf8::ucfirst';      }
    elsif (/\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Eoldutf8::fc';           }
    ...
    elsif (/\G \b chr           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Eoldutf8::chr';               }
    elsif (/\G \b bytes::ord    (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (/\G \b ord           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return $function_ord;              }
    elsif (/\G \b glob          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Eoldutf8::glob';              }
    elsif (/\G \b lc \b         (?! \s* => )                         /oxgc) { $slash = 'm//'; return 'Char::Eoldutf8::lc_';               }
    elsif (/\G \b lcfirst \b    (?! \s* => )                         /oxgc) { $slash = 'm//'; return 'Char::Eoldutf8::lcfirst_';          }
    elsif (/\G \b uc \b         (?! \s* => )                         /oxgc) { $slash = 'm//'; return 'Char::Eoldutf8::uc_';               }
    elsif (/\G \b ucfirst \b    (?! \s* => )                         /oxgc) { $slash = 'm//'; return 'Char::Eoldutf8::ucfirst_';          }
    elsif (/\G \b fc \b         (?! \s* => )                         /oxgc) { $slash = 'm//'; return 'Char::Eoldutf8::fc_';               }
    ...
    elsif (/\G \b chr \b           (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Eoldutf8::chr_';              }
    elsif (/\G \b bytes::ord \b    (?! \s* => )                      /oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (/\G \b ord \b           (?! \s* => )                      /oxgc) { $slash = 'div'; return $function_ord_;             }
    elsif (/\G \b glob \b          (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Eoldutf8::glob_';             }
...
# end of split
        if    (/\G (?= [,;\)\}\]] )          /oxgc) { return 'Char::Eoldutf8::split' . $e;                 }

# split scalar value
        elsif (/\G ( [\$\@\&\*] $qq_scalar ) /oxgc) { return 'Char::Eoldutf8::split' . $e . e_string($1);  }

# split literal space
        elsif (/\G \b qq       (\#) [ ] (\#) /oxgc) { return 'Char::Eoldutf8::split' . $e . qq  {qq$1 $2}; }
        elsif (/\G \b qq (\s*) (\() [ ] (\)) /oxgc) { return 'Char::Eoldutf8::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\{) [ ] (\}) /oxgc) { return 'Char::Eoldutf8::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\[) [ ] (\]) /oxgc) { return 'Char::Eoldutf8::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\<) [ ] (\>) /oxgc) { return 'Char::Eoldutf8::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\S) [ ] (\2) /oxgc) { return 'Char::Eoldutf8::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b q        (\#) [ ] (\#) /oxgc) { return 'Char::Eoldutf8::split' . $e . qq   {q$1 $2}; }
        elsif (/\G \b q  (\s*) (\() [ ] (\)) /oxgc) { return 'Char::Eoldutf8::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\{) [ ] (\}) /oxgc) { return 'Char::Eoldutf8::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\[) [ ] (\]) /oxgc) { return 'Char::Eoldutf8::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\<) [ ] (\>) /oxgc) { return 'Char::Eoldutf8::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\S) [ ] (\2) /oxgc) { return 'Char::Eoldutf8::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G                ' [ ] '    /oxgc) { return 'Char::Eoldutf8::split' . $e . qq     {' '};  }
        elsif (/\G                " [ ] "    /oxgc) { return 'Char::Eoldutf8::split' . $e . qq     {" "};  }
...
# <WILDCARD> --- glob
    ...
    elsif (/\G < ((?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF>\0\a\e\f\n\r\t])+?) > /oxgc) {
        return 'Char::Eoldutf8::glob("' . $1 . '")';
    }
...
# functions of package Char::Eoldutf8
        ...
        elsif ($string =~ /\G \b chop \b                                      /oxgc) { $e_string .= 'Char::Eoldutf8::chop';    $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::index \b                              /oxgc) { $e_string .= 'index';          $slash = 'm//'; }
        elsif ($string =~ /\G \b Char::OldUTF8::index \b                               /oxgc) { $e_string .= 'Char::OldUTF8::index';    $slash = 'm//'; }
        elsif ($string =~ /\G \b index \b                                     /oxgc) { $e_string .= 'Char::Eoldutf8::index';   $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::rindex \b                             /oxgc) { $e_string .= 'rindex';         $slash = 'm//'; }
        elsif ($string =~ /\G \b Char::OldUTF8::rindex \b                              /oxgc) { $e_string .= 'Char::OldUTF8::rindex';   $slash = 'm//'; }
        elsif ($string =~ /\G \b rindex \b                                    /oxgc) { $e_string .= 'Char::Eoldutf8::rindex';  $slash = 'm//'; }
        elsif ($string =~ /\G \b lc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Eoldutf8::lc';      $slash = 'm//'; }
        elsif ($string =~ /\G \b lcfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Eoldutf8::lcfirst'; $slash = 'm//'; }
        elsif ($string =~ /\G \b uc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Eoldutf8::uc';      $slash = 'm//'; }
        elsif ($string =~ /\G \b ucfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Eoldutf8::ucfirst'; $slash = 'm//'; }
        elsif ($string =~ /\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Eoldutf8::fc';      $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b chr           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Eoldutf8::chr';               $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b glob          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Eoldutf8::glob';              $slash = 'm//'; }
        elsif ($string =~ /\G \b lc \b                                              /oxgc) { $e_string .= 'Char::Eoldutf8::lc_';               $slash = 'm//'; }
        elsif ($string =~ /\G \b lcfirst \b                                         /oxgc) { $e_string .= 'Char::Eoldutf8::lcfirst_';          $slash = 'm//'; }
        elsif ($string =~ /\G \b uc \b                                              /oxgc) { $e_string .= 'Char::Eoldutf8::uc_';               $slash = 'm//'; }
        elsif ($string =~ /\G \b ucfirst \b                                         /oxgc) { $e_string .= 'Char::Eoldutf8::ucfirst_';          $slash = 'm//'; }
        elsif ($string =~ /\G \b fc \b                                              /oxgc) { $e_string .= 'Char::Eoldutf8::fc_';               $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b chr \b                                             /oxgc) { $e_string .= 'Char::Eoldutf8::chr_';              $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::ord \b                                      /oxgc) { $e_string .= 'ord';                      $slash = 'div'; }
        elsif ($string =~ /\G \b ord \b                                             /oxgc) { $e_string .= $function_ord_;             $slash = 'div'; }
        elsif ($string =~ /\G \b glob \b                                            /oxgc) { $e_string .= 'Char::Eoldutf8::glob_';             $slash = 'm//'; }
...
# end of split
            if    ($string =~ /\G (?= [,;\)\}\]] )          /oxgc) { return 'Char::Eoldutf8::split' . $e;                                           }

# split scalar value
            elsif ($string =~ /\G ( [\$\@\&\*] $qq_scalar ) /oxgc) { $e_string .= 'Char::Eoldutf8::split' . $e . e_string($1);  next E_STRING_LOOP; }

# split literal space
            elsif ($string =~ /\G \b qq       (\#) [ ] (\#) /oxgc) { $e_string .= 'Char::Eoldutf8::split' . $e . qq  {qq$1 $2}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\() [ ] (\)) /oxgc) { $e_string .= 'Char::Eoldutf8::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\{) [ ] (\}) /oxgc) { $e_string .= 'Char::Eoldutf8::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\[) [ ] (\]) /oxgc) { $e_string .= 'Char::Eoldutf8::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\<) [ ] (\>) /oxgc) { $e_string .= 'Char::Eoldutf8::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\S) [ ] (\2) /oxgc) { $e_string .= 'Char::Eoldutf8::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q        (\#) [ ] (\#) /oxgc) { $e_string .= 'Char::Eoldutf8::split' . $e . qq   {q$1 $2}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\() [ ] (\)) /oxgc) { $e_string .= 'Char::Eoldutf8::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\{) [ ] (\}) /oxgc) { $e_string .= 'Char::Eoldutf8::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\[) [ ] (\]) /oxgc) { $e_string .= 'Char::Eoldutf8::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\<) [ ] (\>) /oxgc) { $e_string .= 'Char::Eoldutf8::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\S) [ ] (\2) /oxgc) { $e_string .= 'Char::Eoldutf8::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G                ' [ ] '    /oxgc) { $e_string .= 'Char::Eoldutf8::split' . $e . qq     {' '};  next E_STRING_LOOP; }
            elsif ($string =~ /\G                " [ ] "    /oxgc) { $e_string .= 'Char::Eoldutf8::split' . $e . qq     {" "};  next E_STRING_LOOP; }
...
sub character_class {
    ...
    if ($char eq '.') {
        if ($modifier =~ /s/) {
            return '${Char::Eoldutf8::dot_s}';
        }
        else {
            return '${Char::Eoldutf8::dot}';
        }
    }
    else {
        return Char::Eoldutf8::classic_character_class($char);
    }
}
...
sub e_tr {
    ...
    if ($modifier =~ tr/bB//d) {
        ...
        if ($variable eq '') {
            $e_tr = qq{Char::Eoldutf8::tr(\$_,' =~ ',$charclass,$e$charclass2,'$modifier')};
        }
        else {
            $e_tr = qq{Char::Eoldutf8::tr($variable,'$bind_operator',$charclass,$e$charclass2,'$modifier')};
        }
    }
...
sub e_qq {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # octal escape sequence
            ...
            $char[$i] = Char::Eoldutf8::octchr($1);
        }

        # hexadecimal escape sequence
        elsif ($char[$i] =~ /\A \\x \{ ([0-9A-Fa-f]+) \} \z/oxms) {
            $char[$i] = Char::Eoldutf8::hexchr($1);
        }
        ...
        # \u \l \U \L \F \Q \E
            ...
            $char[$i] = '@{[Char::Eoldutf8::ucfirst qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\l') {
            $char[$i] = '@{[Char::Eoldutf8::lcfirst qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\U') {
            $char[$i] = '@{[Char::Eoldutf8::uc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\L') {
            $char[$i] = '@{[Char::Eoldutf8::lc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\F') {
            $char[$i] = '@{[Char::Eoldutf8::fc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\Q') {
...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
            ...
            while (1) {
                ...
                if ($char[$i] eq ']') {
                    ...
                    if (grep(/\A [\$\@]/oxms,@char[$left+1..$right-1]) >= 1) {
                        splice @char, $left, $right-$left+1, sprintf(q{@{[Char::Eoldutf8::charlist_qr(%s,'%s')]}}, join(',', map {qq_stuff($delimiter,$end_delimiter,$_)} @char[$left+1..$right-1]), $modifier);
                    }
                    else {
                        splice @char, $left, $right-$left+1, Char::Eoldutf8::charlist_qr(@char[$left+1..$right-1], $modifier);
                    }
        ...
        # open character class [^...]
            ...
            while (1) {
                ...
                if ($char[$i] eq ']') {
                    ...
                    # [^...]
                    if (grep(/\A [\$\@]/oxms,@char[$left+1..$right-1]) >= 1) {
                        splice @char, $left, $right-$left+1, sprintf(q{@{[Char::Eoldutf8::charlist_not_qr(%s,'%s')]}}, join(',', map {qq_stuff($delimiter,$end_delimiter,$_)} @char[$left+1..$right-1]), $modifier);
                    }
                    else {
                        splice @char, $left, $right-$left+1, Char::Eoldutf8::charlist_not_qr(@char[$left+1..$right-1], $modifier);
                    }
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ /\A [\x00-\xFF] \z/oxms) and (Char::Eoldutf8::uc($char[$i]) ne Char::Eoldutf8::fc($char[$i]))) {
            if (CORE::length(Char::Eoldutf8::fc($char[$i])) == 1) {
                $char[$i] = '['   . Char::Eoldutf8::uc($char[$i])       . Char::Eoldutf8::fc($char[$i]) . ']';
            }
            else {
                $char[$i] = '(?:' . Char::Eoldutf8::uc($char[$i]) . '|' . Char::Eoldutf8::fc($char[$i]) . ')';
            }
        ...
        # $0 --> $0
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Eoldutf8::ignorecase(' . $char[$i] . ')]}';
            }
        }
        elsif ($char[$i] =~ /\A \$ \{ \s* 0 \s* \} \z/oxms) {
            if ($ignorecase) {
        ...
        # $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> Char::Eoldutf8::PREMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Eoldutf8::ignorecase(Char::Eoldutf8::PREMATCH())]}';
            }
            else {
                $char[$i] = '@{[Char::Eoldutf8::PREMATCH()]}';
            }
        ...
        # $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> Char::Eoldutf8::MATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Eoldutf8::ignorecase(Char::Eoldutf8::MATCH())]}';
            }
            else {
                $char[$i] = '@{[Char::Eoldutf8::MATCH()]}';
            }
        ...
        # $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> Char::Eoldutf8::POSTMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Eoldutf8::ignorecase(Char::Eoldutf8::POSTMATCH())]}';
            }
            else {
                $char[$i] = '@{[Char::Eoldutf8::POSTMATCH()]}';
            }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $Char::OldUTF8::re_t=%s; while($Char::OldUTF8::re_t =~ %s){%s local $^W=0; local $Char::OldUTF8::re_r=%s; %s$Char::OldUTF8::re_t="$`$Char::OldUTF8::re_r$'"; pos($Char::OldUTF8::re_t)=length "$`$Char::OldUTF8::re_r"; } return $Char::OldUTF8::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Char::OldUTF8::re_r=eval $Char::OldUTF8::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $Char::OldUTF8::re_r=%s; %s"%s$Char::OldUTF8::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $Char::OldUTF8::re_n=0; while(%s =~ %s){%s local $^W=0; local $Char::OldUTF8::re_r=%s; %s%s="$`$Char::OldUTF8::re_r$'"; pos(%s)=length "$`$Char::OldUTF8::re_r"; $Char::OldUTF8::re_n++} return %s$Char::OldUTF8::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $Char::OldUTF8::re_r=%s; %s%s="%s$Char::OldUTF8::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $Char::OldUTF8::re_r=%s; %s%s="%s$Char::OldUTF8::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Char::OldUTF8::re_r=eval $Char::OldUTF8::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
sub e_split {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # ${ foo }
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Eoldutf8::ignorecase(' . $1 . ')]}';
            }
        }
    ...
    $modifier =~ tr/i//d;
    if ($left_e > $right_e) {
        return join '', 'Char::Eoldutf8::split', $ope, $delimiter, @char, '>]}' x ($left_e - $right_e), $end_delimiter, $modifier;
    }
    return     join '', 'Char::Eoldutf8::split', $ope, $delimiter, @char,                               $end_delimiter, $modifier;
}
...
=item * You'll have to download Char/OldUTF8.pm and Char/Eoldutf8.pm and put it in your perl lib directory.

=item * You'll need to write "use Char::OldUTF8;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Char::OldUTF8;
  use Char::OldUTF8 ver.sion;             --- require minimum version
  use Char::OldUTF8 ver.sion.0;           --- expects version (match or die)
  use Char::OldUTF8 qw(ord reverse getc); --- demand enhanced feature of ord, reverse, and getc
  use Char::OldUTF8 ver.sion qw(ord reverse getc);
  use Char::OldUTF8 ver.sion.0 qw(ord reverse getc);

  # "no Char::OldUTF8;" not supported

  or

  $ perl Char/OldUTF8.pm old UTF-8_script.pl > Escaped_script.pl.e

  then

  $ perl Escaped_script.pl.e

  old UTF-8_script.pl  --- script written in old UTF-8
  Escaped_script.pl.e --- escaped script

  functions:
    Char::OldUTF8::ord(...);
    Char::OldUTF8::reverse(...);
    Char::OldUTF8::getc(...);
    Char::OldUTF8::length(...);
    Char::OldUTF8::substr(...);
    Char::OldUTF8::index(...);
...
Char::OldUTF8 software is "middleware" between perl interpreter and your Perl script
written in old UTF-8.

Perl is optimized for problems which are about 90% working with text and about
10% everything else. Even if this "text" doesn't contain old UTF-8, Perl3 or later
can treat old UTF-8 as binary data.

By "use Char::OldUTF8;", it automatically interpret your script as old UTF-8. The various
functions of perl including a regular expression can treat old UTF-8 now.
...
This software is a source code filter to escape Perl script encoded by old UTF-8
...
=item * Remains One Language Handling Raw old UTF-8, Doesn't Use UTF8 flag like JPerl
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Char/OldUTF8.pm, Char/Eoldutf8.pm
  +---------------------------------------+
...
expand wildcard arguments supplied to programs. Instead, import of Char/Eoldutf8.pm
works well.
   ...
   sub import {
       ...
       if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
           ...
           for (@ARGV) {
               ...
               # has space
               if (/\A (?:$q_char)*? [ ] /oxms) {
                   if (my @glob = Char::Eoldutf8::glob(qq{"$_"})) {
                       push @argv, @glob;
                   }
                   else {
                       push @argv, $_;
               ...
               # has wildcard metachar
                   ...
                   if (my @glob = Char::Eoldutf8::glob($_)) {
                       push @argv, @glob;
                   }
                   else {
                       push @argv, $_;
...
=head1 Software Composition
   ...
   Char/OldUTF8.pm               --- source code filter to escape old UTF-8
   Char/Eoldutf8.pm              --- run-time routines for Char/OldUTF8.pm
...
You need write 'use Char::OldUTF8;' in your script.

  ---------------------------------
  Before      You do
  ---------------------------------
  (nothing)   use Char::OldUTF8;
  ---------------------------------
...
=head1 Multiple-Octet Character Regular Expression (Char/OldUTF8.pm provides)

Char/OldUTF8.pm clusters multiple-octet character with quantifier, makes cluster from
...
from classic Perl character class shortcuts and POSIX-style character classes.
  ...
  m/...\D.../             m/...(?:${Char::Eoldutf8::eD}).../
  m/...[[:^digit:]].../   m/...(?:${Char::Eoldutf8::not_digit}).../
  --------------------------------------------------------------------------------

=head1 Calling 'Char::Eoldutf8::ignorecase()' (Char/OldUTF8.pm provides)

Char/OldUTF8.pm applies calling 'Char::Eoldutf8::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Char::Eoldutf8::ignorecase($var)]}.../
  --------------------------------------------------------------------------------
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  Before                  After
  --------------------------------------------------------------------------------
  /regexp/                /ditto$Char::Eoldutf8::matched/
  m/regexp/               m/ditto$Char::Eoldutf8::matched/
  ?regexp?                m?ditto$Char::Eoldutf8::matched?
  m?regexp?               m?ditto$Char::Eoldutf8::matched?
 
  $_ =~                   ($_ =~ m/ditto$Char::Eoldutf8::matched/) ?
  s/regexp/replacement/   eval{ Char::Eoldutf8::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
 
  $_ !~                   ($_ !~ m/ditto$Char::Eoldutf8::matched/) ?
  s/regexp/replacement/   1 :
                          eval{ Char::Eoldutf8::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; undef }
 
  split(/regexp/)         Char::Eoldutf8::split(qr/regexp/)
  split(m/regexp/)        Char::Eoldutf8::split(qr/regexp/)
  split(qr/regexp/)       Char::Eoldutf8::split(qr/regexp/)
  qr/regexp/              qr/ditto$Char::Eoldutf8::matched/
  --------------------------------------------------------------------------------
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  Before                  After
  --------------------------------------------------------------------------------
  /regexp/b               /(?:regexp)$Char::Eoldutf8::matched/
  m/regexp/b              m/(?:regexp)$Char::Eoldutf8::matched/
  ?regexp?b               m?regexp$Char::Eoldutf8::matched?
  m?regexp?b              m?regexp$Char::Eoldutf8::matched?
 
  $_ =~                   ($_ =~ m/(\G[\x00-\xFF]*?)(?:regexp)$Char::Eoldutf8::matched/) ?
  s/regexp/replacement/b  eval{ Char::Eoldutf8::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
  ...
  qr/regexp/b             qr/(?:regexp)$Char::Eoldutf8::matched/
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Char/Eoldutf8.pm provides)
...
The character classes are redefined as follows to backward compatibility.
  ...
  ---------------------------------------------------------------
   .            ${Char::Eoldutf8::dot}
                ${Char::Eoldutf8::dot_s}    (/s modifier)
  \d            [0-9]
  \s            \s
  \w            [0-9A-Z_a-z]
  \D            ${Char::Eoldutf8::eD}
...
Definitions in Char/Eoldutf8.pm.

  ---------------------------------------------------------------------------------------------------------------------------------------------------------
  After                    Definition
  ---------------------------------------------------------------------------------------------------------------------------------------------------------
  ${Char::Eoldutf8::dot}            qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\x0A])}
  ${Char::Eoldutf8::dot_s}          qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF])}
  ${Char::Eoldutf8::eD}             qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF0-9])}
  ${Char::Eoldutf8::eS}             qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\s])}
  ${Char::Eoldutf8::eW}             qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF0-9A-Z_a-z])}
  ${Char::Eoldutf8::eH}             qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\x09\x20])}
  ${Char::Eoldutf8::eV}             qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\x0A\x0B\x0C\x0D])}
  ${Char::Eoldutf8::eR}             qr{(?:\x0D\x0A|[\x0A\x0D])}
  ${Char::Eoldutf8::eN}             qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\x0A])}
  ${Char::Eoldutf8::not_alnum}      qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\x30-\x39\x41-\x5A\x61-\x7A])}
  ${Char::Eoldutf8::not_alpha}      qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\x41-\x5A\x61-\x7A])}
  ${Char::Eoldutf8::not_ascii}      qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\x00-\x7F])}
  ${Char::Eoldutf8::not_blank}      qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\x09\x20])}
  ${Char::Eoldutf8::not_cntrl}      qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\x00-\x1F\x7F])}
  ${Char::Eoldutf8::not_digit}      qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\x30-\x39])}
  ${Char::Eoldutf8::not_graph}      qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\x21-\x7F])}
  ${Char::Eoldutf8::not_lower}      qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\x61-\x7A])}
  ${Char::Eoldutf8::not_lower_i}    qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF])}
  ${Char::Eoldutf8::not_print}      qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\x20-\x7F])}
  ${Char::Eoldutf8::not_punct}      qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\x21-\x2F\x3A-\x3F\x40\x5B-\x5F\x60\x7B-\x7E])}
  ${Char::Eoldutf8::not_space}      qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\s\x0B])}
  ${Char::Eoldutf8::not_upper}      qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\x41-\x5A])}
  ${Char::Eoldutf8::not_upper_i}    qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF])}
  ${Char::Eoldutf8::not_word}       qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\x30-\x39\x41-\x5A\x5F\x61-\x7A])}
  ${Char::Eoldutf8::not_xdigit}     qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\x30-\x39\x41-\x46\x61-\x66])}
...
=head1 Un-Escaping \ of \N, \p, \P and \X (Char/OldUTF8.pm provides)

Char/OldUTF8.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Char/OldUTF8.pm and Char/Eoldutf8.pm provide)

Insert 'Char::Eoldutf8::' at head of function name. Char/Eoldutf8.pm provides your script Char::Eoldutf8::*
functions.
  ...
  substr      substr           Byte
  pos         pos              Byte
  split       Char::Eoldutf8::split     Character
  tr///       Char::Eoldutf8::tr        Character
  tr///b      tr///            Byte
  tr///B      tr///            Byte
  y///        Char::Eoldutf8::tr        Character
  y///b       tr///            Byte
  y///B       tr///            Byte
  chop        Char::Eoldutf8::chop      Character
  lc          Char::Eoldutf8::lc        Character
  lcfirst     Char::Eoldutf8::lcfirst   Character
  uc          Char::Eoldutf8::uc        Character
  ucfirst     Char::Eoldutf8::ucfirst   Character
  fc          Char::Eoldutf8::fc        Character
  chr         Char::Eoldutf8::chr       Character
  glob        Char::Eoldutf8::glob      Character
  -------------------------------------------
...
You need write 'Char::OldUTF8::' at head of function name when you want character-
oriented function. See 'Character-Oriented Functions'.

  --------------------------------------------------------
  Function   Character-Oriented   Description
  --------------------------------------------------------
  ord        Char::OldUTF8::ord
  reverse    Char::OldUTF8::reverse
  getc       Char::OldUTF8::getc
  length     Char::OldUTF8::length
  substr     Char::OldUTF8::substr
  index      Char::OldUTF8::index          See 'About Indexes'
  rindex     Char::OldUTF8::rindex         See 'About Rindexes'
  --------------------------------------------------------
  ...
  (same as Char::Eoldutf8::index)
  Char::OldUTF8::index    Character   Character    Character-oriented semantics
  ...
  (same as Char::Eoldutf8::rindex)
  Char::OldUTF8::rindex   Character   Character    Character-oriented semantics
...
=item * Ordinal Value of Character
  ...
  $ord = Char::OldUTF8::ord($string);

  This function returns the numeric value (ASCII or old UTF-8 character) of the
  first character of $string, not Unicode. If $string is omitted, it uses $_.
  The return value is always unsigned.

  If you import ord "use Char::OldUTF8 qw(ord);", ord of your script will be rewritten in
  Char::OldUTF8::ord. Char::OldUTF8::ord is not compatible with ord of JPerl.

=item * Reverse List or String

  @reverse = Char::OldUTF8::reverse(@list);
  $reverse = Char::OldUTF8::reverse(@list);
  ...
  If you import reverse "use Char::OldUTF8 qw(reverse);", reverse of your script will be
  rewritten in Char::OldUTF8::reverse. Char::OldUTF8::reverse is not compatible with reverse of
  JPerl.
...
=item * Returns Next Character
  ...
  $getc = Char::OldUTF8::getc(FILEHANDLE);
  $getc = Char::OldUTF8::getc($filehandle);
  $getc = Char::OldUTF8::getc;
  ...
  If you import getc "use Char::OldUTF8 qw(getc);", getc of your script will be rewritten in
  Char::OldUTF8::getc. Char::OldUTF8::getc is not compatible with getc of JPerl.

=item * Length by old UTF-8 Character

  $length = Char::OldUTF8::length($string);
  $length = Char::OldUTF8::length();

  This function returns the length in characters (programmer-visible characters) of
  the scalar value $string. If $string is omitted, it returns the Char::OldUTF8::length of
  $_.

  Do not try to use Char::OldUTF8::length to find the size of an array or hash. Use scalar
...
=item * Substr by old UTF-8 Character

  $substr = Char::OldUTF8::substr($string,$offset,$length,$replacement);
  $substr = Char::OldUTF8::substr($string,$offset,$length);
  $substr = Char::OldUTF8::substr($string,$offset);
  ...
  my $color  = Char::OldUTF8::substr $s, 4, 5;      # black
  my $middle = Char::OldUTF8::substr $s, 4, -11;    # black cat climbed the
  my $end    = Char::OldUTF8::substr $s, 14;        # climbed the green tree
  my $tail   = Char::OldUTF8::substr $s, -4;        # tree
  my $z      = Char::OldUTF8::substr $s, -4, 2;     # tr

  If Perl version 5.14 or later, you can use the Char::OldUTF8::substr() function as an
  ...
  end of the string, Char::OldUTF8::substr() returns the undefined value and produces a
  ...
  my $name = 'fred';
  Char::OldUTF8::substr($name, 4) = 'dy';         # $name is now 'freddy'
  my $null = Char::OldUTF8::substr $name, 6, 2;   # returns "" (no warning)
  my $oops = Char::OldUTF8::substr $name, 7;      # returns undef, with warning
  Char::OldUTF8::substr($name, 7) = 'gap';        # raises an exception

  An alternative to using Char::OldUTF8::substr() as an lvalue is to specify the replacement
  ...
  my $s = "The black cat climbed the green tree";
  my $z = Char::OldUTF8::substr $s, 14, 7, "jumped from";    # climbed
  # $s is now "The black cat jumped from the green tree"

  Note that the lvalue returned by the three-argument version of Char::OldUTF8::substr() acts
...
=item * Index by old UTF-8 Character

  $index = Char::OldUTF8::index($string,$substring,$offset);
  $index = Char::OldUTF8::index($string,$substring);
  ...
  $pos = -1;
  while (($pos = Char::OldUTF8::index($string, $lookfor, $pos)) > -1) {
      print "Found at $pos\n";
      $pos++;
  }

=item * Rindex by old UTF-8 Character

  $rindex = Char::OldUTF8::rindex($string,$substring,$offset);
  $rindex = Char::OldUTF8::rindex($string,$substring);

  This function works just like Char::OldUTF8::index except that it returns the character
  position of the last occurrence of $substring in $string (a reverse Char::OldUTF8::index).
  ...
  $pos = Char::OldUTF8::length($string);
  while (($pos = Char::OldUTF8::rindex($string, $lookfor, $pos)) >= 0) {
      print "Found at $pos\n";
      $pos--;
  }
...
=item * Filename Globbing
  ...
  This operator is implemented via the Char::Eoldutf8::glob() function. See Char::Eoldutf8::glob of
  Char/Eoldutf8.pm for details.
...
=item * Ordinal Value of Byte
  ...
  of "use Char::OldUTF8 qw(ord);" exists or not. If $expr is omitted, it uses $_.
  The return value is always unsigned.
...
=item * Reverse List or Byte String
  ...
  "use Char::OldUTF8 qw(reverse);" exists or not.
...
=head1 Un-Escaping bytes::* Functions (Char/OldUTF8.pm provides)

Char/OldUTF8.pm removes 'bytes::' at head of function name.
...
=head1 Escaping Built-in Standard Module (Char/Eoldutf8.pm provides)

Char/Eoldutf8.pm does "BEGIN { unshift @INC, '/Perl/site/lib/Char::OldUTF8' }" at head.
Store the standard module modified for Char::OldUTF8 software in this directory to
override built-in standard modules.

=head1 Escaping Standard Module Content (You do)

You need copy built-in standard module to /Perl/site/lib/Char::OldUTF8 and change
'use utf8;' to 'use Char::OldUTF8;' in its. You need help yourself for now.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Char/Eoldutf8.pm provides these
  functions.
...
=item * Char::OldUTF8::substr as Lvalue

Char::OldUTF8::substr differs from CORE::substr, and cannot be used as a lvalue.
To change part of a string, you can use the optional fourth argument which is the
replacement string.

Char::OldUTF8::substr($string, 13, 4, "JPerl");
...
Char::OldUTF8 software remains one language and one interpreter.
