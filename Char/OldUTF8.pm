...
my $matched     = '';
my $s_matched   = '';
$matched        = q{$Char::Eoldutf8::matched};
$s_matched      = q{ Char::Eoldutf8::s_matched();};
...
# while (<WILDCARD>) --- glob
    ...
    elsif (/\G \b while \s* \( \s* < ((?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x00-\xFF]|[^\x80-\xFF>\0\a\e\f\n\r\t])+?) > \s* \) \b /oxgc) {
...
# <WILDCARD> --- glob
    ...
    elsif (/\G < ((?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x00-\xFF]|[^\x80-\xFF>\0\a\e\f\n\r\t])+?) > /oxgc) {
...
sub e_capture {
    ...
    return join '', '${Char::Eoldutf8::capture(', $_[0], ')}';
    return join '', '$',                 $_[0];
}
...
written by old UTF-8.

Perl is optimized for problems which are about 90% working with text and about
10% everything else. But this "text" means US-ASCII text, and popular old UTF-8
...
Perl3 or later can treat old UTF-8.

Perl is designed to make the easy jobs easy, without making the hard jobs
impossible. Char::OldUTF8 software is Perl program designed to make the "easy jobs easy".
...
=item * Handles Raw old UTF-8, Doesn't use UTF8 flag like JPerl
...
=head1 Multiple-Octet Anchoring of Regular Expression (Char/OldUTF8.pm provides)

Char/OldUTF8.pm applies multiple-octet anchoring at beginning of regular expression.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/regexp/               m/${Char::Eoldutf8::anchor}(?:regexp).../
  --------------------------------------------------------------------------------
...
Definitions in Char/Eoldutf8.pm.
  ...
  ${Char::Eoldutf8::eV}             qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x00-\xFF]|[^\x80-\xFF\x0A\x0B\x0C\x0D])}
...
functions.
  ...
  index       Char::Eoldutf8::index     Character
  rindex      Char::Eoldutf8::rindex    Character
...
=item * Special Variables $` and $& need /( Capture All )/
  ...
  Before          After                Works as
  -------------------------------------------------------------------------------------------
  $`              Char::Eoldutf8::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $PREMATCH       Char::Eoldutf8::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  ${^PREMATCH}    Char::Eoldutf8::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $&              Char::Eoldutf8::MATCH()       $1
  $MATCH          Char::Eoldutf8::MATCH()       $1
  ${^MATCH}       Char::Eoldutf8::MATCH()       $1
  $'              Char::Eoldutf8::POSTMATCH()   $'
  $POSTMATCH      Char::Eoldutf8::POSTMATCH()   $'
  ${^POSTMATCH}   Char::Eoldutf8::POSTMATCH()   $'
  -------------------------------------------------------------------------------------------
...
It has already been achieved by old UTF-8 designed for combining with
...
Contrasting is Encode module and describing "use Char::OldUTF8;" on this software,
