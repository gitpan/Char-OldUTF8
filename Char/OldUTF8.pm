...
# regexp of character
my $qq_char   = qr/\\c[\x40-\x5F]|\\?(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[\x00-\x7F\xF5-\xFF])/oxms;
my  $q_char   = qr/(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[\x00-\x7F\xF5-\xFF]/oxms;
...
END
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                for (@list) {
                    $Char::Eoldutf8::function_ord     = 'Char::OldUTF8::ord'     if /\A ord \z/oxms;
                    $Char::Eoldutf8::function_ord_    = 'Char::OldUTF8::ord_'    if /\A ord \z/oxms;
                    $Char::Eoldutf8::function_reverse = 'Char::OldUTF8::reverse' if /\A reverse \z/oxms;
                    $Char::Eoldutf8::function_getc    = 'Char::OldUTF8::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }

    $e_script .= Char::OldUTF8::escape();
...
=head1 Multiple-Octet Character Regular Expression (Char::OldUTF8 software provides)

Char::OldUTF8 software clusters multiple-octet character with quantifier, makes cluster from
...
=head1 Calling 'Char::Eoldutf8::ignorecase()' (Char::OldUTF8 software provides)

Char::OldUTF8 software applies calling 'Char::Eoldutf8::ignorecase()' instead of /i modifier.
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  s/regexp/replacement/   CORE::eval{ Char::Eoldutf8::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
 
  $_ !~                   ($_ !~ m/ditto$Char::Eoldutf8::matched/) ?
  s/regexp/replacement/   1 :
                          CORE::eval{ Char::Eoldutf8::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; undef }
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  s/regexp/replacement/b  CORE::eval{ Char::Eoldutf8::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
...
=head1 Un-Escaping \ of \N, \p, \P, and \X (Char::OldUTF8 software provides)

Char::OldUTF8 software removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Char::OldUTF8 software provide)
...
=item * Eval old UTF-8 Script

  $eval = Char::OldUTF8::eval { block };
  $eval = Char::OldUTF8::eval $expr;
  $eval = Char::OldUTF8::eval;

  The Char::OldUTF8::eval keyword serves two distinct but related purposes in JPerl.
  These purposes are represented by two forms of syntax, Char::OldUTF8::eval { block }
  and Char::OldUTF8::eval $expr. The first form traps runtime exceptions (errors)
  ...
  more general. Whichever form you use, Char::OldUTF8::eval is the preferred way to do
  all exception handling in JPerl.

  For either form of Char::OldUTF8::eval, the value returned from an Char::OldUTF8::eval is
  ...
  Char::OldUTF8::eval itself. See wantarray for more on how the evaluation context
  can be determined.

  If there is a trappable error (including any produced by the die operator),
  Char::OldUTF8::eval returns undef and puts the error message (or object) in $@. If
  ...
  can test it reliably afterward for errors. A simple Boolean test suffices:
      ...
      Char::OldUTF8::eval { ... }; # trap runtime errors
      if ($@) { ... }     # handle error

  (Prior to Perl 5.16, a bug caused undef to be returned in list context for
  syntax errors, but not for runtime errors.)

  The Char::OldUTF8::eval { block } form is syntax checked and compiled at compile time,
  so it is just as efficient at runtime as any other block. (People familiar
  with the slow Char::OldUTF8::eval $expr form are occasionally confused on this issue.)
  Because the { block } is compiled when the surrounding code is, this form of
  Char::OldUTF8::eval cannot trap syntax errors.

  The Char::OldUTF8::eval $expr form can trap syntax errors because it parses the code
  ...
  after the Char::OldUTF8::eval is complete, as do any subroutine or format definitions.
  The code of the Char::OldUTF8::eval is treated as a block, so any locally scoped
  variables declared within the Char::OldUTF8::eval last only until the Char::OldUTF8::eval is
  done. (See my and local.) As with any code in a block, a final semicolon is
  not required.

  Char::OldUTF8::eval will be escaped as follows:

  -------------------------------------------------
  Before                  After
  -------------------------------------------------
  Char::OldUTF8::eval { block }    eval { block }
  Char::OldUTF8::eval $expr        eval Char::OldUTF8::escape $expr
  Char::OldUTF8::eval              eval Char::OldUTF8::escape
  -------------------------------------------------

  To tell the truth, the subroutine Char::OldUTF8::eval does not exist. If it exists,
  you will troubled, when Char::OldUTF8::eval has a parameter that is single quoted
  string included my variables. Char::OldUTF8::escape is a subroutine that makes Perl
  ...
  Since Char::OldUTF8::eval traps errors that would otherwise prove fatal, it is useful
  for determining whether particular features (such as fork or symlink) are
  implemented.

  Because Char::OldUTF8::eval { block } is syntax checked at compile time, any syntax
  error is reported earlier. Therefore, if your code is invariant and both
  Char::OldUTF8::eval $expr and Char::OldUTF8::eval { block } will suit your purposes equally
  well, the { block } form is preferred. For example:

      # make divide-by-zero nonfatal
      Char::OldUTF8::eval { $answer = $a / $b; };
      warn $@ if $@;
      ...
      Char::OldUTF8::eval { $answer = }; # WRONG

      # a runtime syntax error
      Char::OldUTF8::eval '$answer =';   # sets $@
  ...
  Using the Char::OldUTF8::eval { block } form as an exception trap in libraries does
  ...
  as this example shows:
      ...
      Char::OldUTF8::eval { local $SIG{'__DIE__'}; $answer = $a / $b; };
      warn $@ if $@;
  ...
  which has the effect of changing their error messages:
      ...
      # __DIE__ hooks may modify error messages
      {
          ...
          Char::OldUTF8::eval { die "foo lives here" };
  ...
  With an Char::OldUTF8::eval, you should be especially careful to remember what's being
  looked at when:

      Char::OldUTF8::eval $x;        # CASE 1
      Char::OldUTF8::eval "$x";      # CASE 2

      Char::OldUTF8::eval '$x';      # CASE 3
      Char::OldUTF8::eval { $x };    # CASE 4

      Char::OldUTF8::eval "\$$x++";  # CASE 5
      $$x++;                # CASE 6
  ...
  a temporary is required if you want to mask some but not all errors:
      ...
      # alter $@ on nefarious repugnancy only
      {
          my $e;
          {
              ...
              Char::OldUTF8::eval { test_repugnancy() };
  ...
  The block of Char::OldUTF8::eval { block } does not count as a loop, so the loop
...
=head1 Un-Escaping bytes::* Subroutines (Char::OldUTF8 software provides)

Char::OldUTF8 software removes 'bytes::' at head of subroutine name.
...
some bugs. Thus, if you happen to find a bug that's in Char::OldUTF8 software and
