...
######################################################################
#
# OldUTF8 - Source code filter to escape old UTF-8 script
#
...
END
    ...
    print OldUTF8::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        ...
        if (CORE::eval q{ use Fcntl qw(O_WRONLY O_APPEND O_CREAT); 1 } and CORE::sysopen($fh,"$filename.e",&O_WRONLY|&O_APPEND|&O_CREAT)) {
            ...
            Eoldutf8::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e";
        }
    ...
    my $fh = gensym();
    Eoldutf8::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e";
...
sub OldUTF8::escape_script {
    ...
    my $fh = gensym();
    Eoldutf8::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die __FILE__, ": Can't close file: $script";

    if (/^ use Eoldutf8(?:\s+[0-9\.]*)?\s*; $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Eoldutf8 '%s.0'; # 'quote' for perl5.6\n", $OldUTF8::VERSION); # require run-time routines version

        # use OldUTF8 version qw(ord reverse getc);
        if (s/^ \s* use \s+ OldUTF8 \s* ([^\x80-\xFF;]*) ; \s* \n? $//oxms) {

            # require version
            my $list = $1;
            if ($list =~ s/\A ([0-9]+\.[0-9]+) \.0 \s* //oxms) {
                my $version = $1;
                if ($version ne $OldUTF8::VERSION) {
                    my @file = grep -e, map {qq{$_/OldUTF8.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects OldUTF8.pm $version, but @{[__FILE__]} is version $OldUTF8::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Eoldutf8.pm $version, but \\\$Eoldutf8::VERSION is \$Eoldutf8::VERSION" if \$Eoldutf8::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ([0-9]+(?:\.[0-9]*)) \s* //oxms) {
                my $version = $1;
                if ($version > $OldUTF8::VERSION) {
                    die "Script $0 required OldUTF8.pm $version, but @{[__FILE__]} is only version $OldUTF8::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                for (@list) {
                    $Eoldutf8::function_ord     = 'OldUTF8::ord'     if /\A ord \z/oxms;
                    $Eoldutf8::function_ord_    = 'OldUTF8::ord_'    if /\A ord \z/oxms;
                    $Eoldutf8::function_reverse = 'OldUTF8::reverse' if /\A reverse \z/oxms;
                    $Eoldutf8::function_getc    = 'OldUTF8::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }

    $e_script .= OldUTF8::escape();
...
=item * You'll have to download OldUTF8.pm and Eoldutf8.pm and put it in your perl lib directory.

=item * You'll need to write "use OldUTF8;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use OldUTF8;
  use OldUTF8 ver.sion;             --- require minimum version
  use OldUTF8 ver.sion.0;           --- expects version (match or die)
  use OldUTF8 qw(ord reverse getc); --- demand enhanced feature of ord, reverse, and getc
  use OldUTF8 ver.sion qw(ord reverse getc);
  use OldUTF8 ver.sion.0 qw(ord reverse getc);

  # "no OldUTF8;" not supported

  or

  $ perl OldUTF8.pm old UTF-8_script.pl > Escaped_script.pl.e
...
OldUTF8 software is "middleware" between perl interpreter and your Perl script
...
By "use OldUTF8;", it automatically interpret your script as old UTF-8. The various
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. OldUTF8.pm, Eoldutf8.pm
  +---------------------------------------+
...
import of Eoldutf8.pm works well.
...
=head1 Software Composition
   ...
   OldUTF8.pm               --- source code filter to escape old UTF-8
   Eoldutf8.pm              --- run-time routines for OldUTF8.pm
...
You need write 'use OldUTF8;' in your script.
...
=head1 Multiple-Octet Character Regular Expression (OldUTF8 software provides)

OldUTF8 software clusters multiple-octet character with quantifier, makes cluster from
...
from classic Perl character class shortcuts and POSIX-style character classes.
  ...
  m/...\D.../             m/...(?:${Eoldutf8::eD}).../
  m/...[[:^digit:]].../   m/...(?:${Eoldutf8::not_digit}).../
  --------------------------------------------------------------------------------

=head1 Calling 'Eoldutf8::ignorecase()' (OldUTF8 software provides)

OldUTF8 software applies calling 'Eoldutf8::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Eoldutf8::ignorecase($var)]}.../
  --------------------------------------------------------------------------------
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  Before                  After
  --------------------------------------------------------------------------------
  /regexp/                /ditto$Eoldutf8::matched/
  m/regexp/               m/ditto$Eoldutf8::matched/
  ?regexp?                m?ditto$Eoldutf8::matched?
  m?regexp?               m?ditto$Eoldutf8::matched?
 
  $_ =~                   ($_ =~ m/ditto$Eoldutf8::matched/) ?
  s/regexp/replacement/   CORE::eval{ Eoldutf8::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
 
  $_ !~                   ($_ !~ m/ditto$Eoldutf8::matched/) ?
  s/regexp/replacement/   1 :
                          CORE::eval{ Eoldutf8::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; undef }
 
  split(/regexp/)         Eoldutf8::split(qr/regexp/)
  split(m/regexp/)        Eoldutf8::split(qr/regexp/)
  split(qr/regexp/)       Eoldutf8::split(qr/regexp/)
  qr/regexp/              qr/ditto$Eoldutf8::matched/
  --------------------------------------------------------------------------------
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  Before                  After
  --------------------------------------------------------------------------------
  /regexp/b               /(?:regexp)$Eoldutf8::matched/
  m/regexp/b              m/(?:regexp)$Eoldutf8::matched/
  ?regexp?b               m?regexp$Eoldutf8::matched?
  m?regexp?b              m?regexp$Eoldutf8::matched?
 
  $_ =~                   ($_ =~ m/(\G[\x00-\xFF]*?)(?:regexp)$Eoldutf8::matched/) ?
  s/regexp/replacement/b  CORE::eval{ Eoldutf8::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
  ...
  qr/regexp/b             qr/(?:regexp)$Eoldutf8::matched/
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Eoldutf8.pm provides)
...
Definitions in Eoldutf8.pm.

  ---------------------------------------------------------------------------------------------------------------------------------------------------------
  After                    Definition
  ---------------------------------------------------------------------------------------------------------------------------------------------------------
  ${Eoldutf8::dot}            qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\x0A])}
  ${Eoldutf8::dot_s}          qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF])}
  ${Eoldutf8::eD}             qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF0-9])}
  ${Eoldutf8::eS}             qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\s])}
  ${Eoldutf8::eW}             qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF0-9A-Z_a-z])}
  ${Eoldutf8::eH}             qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\x09\x20])}
  ${Eoldutf8::eV}             qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\x0A\x0B\x0C\x0D])}
  ${Eoldutf8::eR}             qr{(?:\x0D\x0A|[\x0A\x0D])}
  ${Eoldutf8::eN}             qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\x0A])}
  ${Eoldutf8::not_alnum}      qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\x30-\x39\x41-\x5A\x61-\x7A])}
  ${Eoldutf8::not_alpha}      qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\x41-\x5A\x61-\x7A])}
  ${Eoldutf8::not_ascii}      qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\x00-\x7F])}
  ${Eoldutf8::not_blank}      qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\x09\x20])}
  ${Eoldutf8::not_cntrl}      qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\x00-\x1F\x7F])}
  ${Eoldutf8::not_digit}      qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\x30-\x39])}
  ${Eoldutf8::not_graph}      qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\x21-\x7F])}
  ${Eoldutf8::not_lower}      qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\x61-\x7A])}
  ${Eoldutf8::not_lower_i}    qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF])}
  ${Eoldutf8::not_print}      qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\x20-\x7F])}
  ${Eoldutf8::not_punct}      qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\x21-\x2F\x3A-\x3F\x40\x5B-\x5F\x60\x7B-\x7E])}
  ${Eoldutf8::not_space}      qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\s\x0B])}
  ${Eoldutf8::not_upper}      qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\x41-\x5A])}
  ${Eoldutf8::not_upper_i}    qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF])}
  ${Eoldutf8::not_word}       qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\x30-\x39\x41-\x5A\x5F\x61-\x7A])}
  ${Eoldutf8::not_xdigit}     qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x80-\xBF]|[^\x80-\xFF\x30-\x39\x41-\x46\x61-\x66])}
...
=head1 Un-Escaping \ of \N, \p, \P, and \X (OldUTF8 software provides)

OldUTF8 software removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (OldUTF8 software provide)

Insert 'Eoldutf8::' at head of function name. Eoldutf8.pm provides your script Eoldutf8::*
subroutines.
  ...
  substr      substr           Byte
  pos         pos              Byte
  split       Eoldutf8::split     Character
  tr///       Eoldutf8::tr        Character
  tr///b      tr///            Byte
  tr///B      tr///            Byte
  y///        Eoldutf8::tr        Character
  y///b       tr///            Byte
  y///B       tr///            Byte
  chop        Eoldutf8::chop      Character
  lc          Eoldutf8::lc        Character
  lcfirst     Eoldutf8::lcfirst   Character
  uc          Eoldutf8::uc        Character
  ucfirst     Eoldutf8::ucfirst   Character
  fc          Eoldutf8::fc        Character
  chr         Eoldutf8::chr       Character
  glob        Eoldutf8::glob      Character
  -------------------------------------------
...
You need write 'OldUTF8::' at head of function name when you want character-
oriented subroutine. See 'Character-Oriented Subroutines'.
  ...
  --------------------------------------------------------
  ord        OldUTF8::ord
  reverse    OldUTF8::reverse
  getc       OldUTF8::getc
  length     OldUTF8::length
  substr     OldUTF8::substr
  index      OldUTF8::index          See 'About Indexes'
  rindex     OldUTF8::rindex         See 'About Rindexes'
  --------------------------------------------------------
  ...
  (same as Eoldutf8::index)
  OldUTF8::index    Character   Character    Character-oriented semantics
  ...
  (same as Eoldutf8::rindex)
  OldUTF8::rindex   Character   Character    Character-oriented semantics
...
=item * Ordinal Value of Character
  ...
  If you import ord "use OldUTF8 qw(ord);", ord of your script will be rewritten in
  OldUTF8::ord. OldUTF8::ord is not compatible with ord of JPerl.

=item * Reverse List or String

  @reverse = OldUTF8::reverse(@list);
  $reverse = OldUTF8::reverse(@list);
  ...
  If you import reverse "use OldUTF8 qw(reverse);", reverse of your script will be
  rewritten in OldUTF8::reverse. OldUTF8::reverse is not compatible with reverse of
  JPerl.
...
=item * Returns Next Character
  ...
  If you import getc "use OldUTF8 qw(getc);", getc of your script will be rewritten
  in OldUTF8::getc. OldUTF8::getc is not compatible with getc of JPerl.

=item * Length by old UTF-8 Character

  $length = OldUTF8::length($string);
  $length = OldUTF8::length();

  This subroutine returns the length in characters (programmer-visible characters)
  of the scalar value $string. If $string is omitted, it returns the OldUTF8::length
  of $_.

  Do not try to use OldUTF8::length to find the size of an array or hash. Use scalar
...
=item * Substr by old UTF-8 Character
  ...
  $substr = OldUTF8::substr($string,$offset,$length,$replacement);
  $substr = OldUTF8::substr($string,$offset,$length);
  $substr = OldUTF8::substr($string,$offset);
  ...
  my $color  = OldUTF8::substr $s, 4, 5;      # black
  my $middle = OldUTF8::substr $s, 4, -11;    # black cat climbed the
  my $end    = OldUTF8::substr $s, 14;        # climbed the green tree
  my $tail   = OldUTF8::substr $s, -4;        # tree
  my $z      = OldUTF8::substr $s, -4, 2;     # tr

  If Perl version 5.14 or later, you can use the OldUTF8::substr() subroutine as an
  ...
  end of the string, OldUTF8::substr() returns the undefined value and produces a
  ...
  my $name = 'fred';
  OldUTF8::substr($name, 4) = 'dy';         # $name is now 'freddy'
  my $null = OldUTF8::substr $name, 6, 2;   # returns "" (no warning)
  my $oops = OldUTF8::substr $name, 7;      # returns undef, with warning
  OldUTF8::substr($name, 7) = 'gap';        # raises an exception

  An alternative to using OldUTF8::substr() as an lvalue is to specify the replacement
  ...
  Note that the lvalue returned by the three-argument version of OldUTF8::substr() acts
...
=item * Index by old UTF-8 Character
  ...
  $index = OldUTF8::index($string,$substring,$offset);
  $index = OldUTF8::index($string,$substring);
  ...
  $pos = -1;
  while (($pos = OldUTF8::index($string, $lookfor, $pos)) > -1) {
      print "Found at $pos\n";
      $pos++;
  }
...
=item * Rindex by old UTF-8 Character
  ...
  $rindex = OldUTF8::rindex($string,$substring,$offset);
  $rindex = OldUTF8::rindex($string,$substring);

  This subroutine works just like OldUTF8::index except that it returns the character
  position of the last occurrence of $substring in $string (a reverse OldUTF8::index).
  ...
  $pos = OldUTF8::length($string);
  while (($pos = OldUTF8::rindex($string, $lookfor, $pos)) >= 0) {
      print "Found at $pos\n";
      $pos--;
  }
...
=item * Eval old UTF-8 Script
  ...
  $eval = OldUTF8::eval { block };
  $eval = OldUTF8::eval $expr;
  $eval = OldUTF8::eval;

  The OldUTF8::eval keyword serves two distinct but related purposes in JPerl.
  These purposes are represented by two forms of syntax, OldUTF8::eval { block }
  and OldUTF8::eval $expr. The first form traps runtime exceptions (errors)
  ...
  more general. Whichever form you use, OldUTF8::eval is the preferred way to do
  all exception handling in JPerl.

  For either form of OldUTF8::eval, the value returned from an OldUTF8::eval is
  ...
  OldUTF8::eval itself. See wantarray for more on how the evaluation context
  can be determined.

  If there is a trappable error (including any produced by the die operator),
  OldUTF8::eval returns undef and puts the error message (or object) in $@. If
  ...
  can test it reliably afterward for errors. A simple Boolean test suffices:
      ...
      OldUTF8::eval { ... }; # trap runtime errors
      if ($@) { ... }     # handle error

  (Prior to Perl 5.16, a bug caused undef to be returned in list context for
  syntax errors, but not for runtime errors.)

  The OldUTF8::eval { block } form is syntax checked and compiled at compile time,
  so it is just as efficient at runtime as any other block. (People familiar
  with the slow OldUTF8::eval $expr form are occasionally confused on this issue.)
  Because the { block } is compiled when the surrounding code is, this form of
  OldUTF8::eval cannot trap syntax errors.

  The OldUTF8::eval $expr form can trap syntax errors because it parses the code
  ...
  after the OldUTF8::eval is complete, as do any subroutine or format definitions.
  The code of the OldUTF8::eval is treated as a block, so any locally scoped
  variables declared within the OldUTF8::eval last only until the OldUTF8::eval is
  done. (See my and local.) As with any code in a block, a final semicolon is
  not required.

  OldUTF8::eval will be escaped as follows:

  -------------------------------------------------
  Before                  After
  -------------------------------------------------
  OldUTF8::eval { block }    eval { block }
  OldUTF8::eval $expr        eval OldUTF8::escape $expr
  OldUTF8::eval              eval OldUTF8::escape
  -------------------------------------------------

  To tell the truth, the subroutine OldUTF8::eval does not exist. If it exists,
  you will troubled, when OldUTF8::eval has a parameter that is single quoted
  string included my variables. OldUTF8::escape is a subroutine that makes Perl
  ...
  Since OldUTF8::eval traps errors that would otherwise prove fatal, it is useful
  for determining whether particular features (such as fork or symlink) are
  implemented.

  Because OldUTF8::eval { block } is syntax checked at compile time, any syntax
  error is reported earlier. Therefore, if your code is invariant and both
  OldUTF8::eval $expr and OldUTF8::eval { block } will suit your purposes equally
  well, the { block } form is preferred. For example:

      # make divide-by-zero nonfatal
      OldUTF8::eval { $answer = $a / $b; };
      warn $@ if $@;
      ...
      OldUTF8::eval { $answer = }; # WRONG

      # a runtime syntax error
      OldUTF8::eval '$answer =';   # sets $@
  ...
  Using the OldUTF8::eval { block } form as an exception trap in libraries does
  ...
  as this example shows:
      ...
      OldUTF8::eval { local $SIG{'__DIE__'}; $answer = $a / $b; };
      warn $@ if $@;
  ...
  which has the effect of changing their error messages:
      ...
      # __DIE__ hooks may modify error messages
      {
          ...
          OldUTF8::eval { die "foo lives here" };
  ...
  With an OldUTF8::eval, you should be especially careful to remember what's being
  looked at when:

      OldUTF8::eval $x;        # CASE 1
      OldUTF8::eval "$x";      # CASE 2

      OldUTF8::eval '$x';      # CASE 3
      OldUTF8::eval { $x };    # CASE 4

      OldUTF8::eval "\$$x++";  # CASE 5
      $$x++;                # CASE 6
  ...
  The block of OldUTF8::eval { block } does not count as a loop, so the loop
...
=item * Filename Globbing
  ...
  This operator is implemented via the Eoldutf8::glob() subroutine. See Eoldutf8::glob
  of Eoldutf8.pm for details.
...
=item * Ordinal Value of Byte
  ...
  of "use OldUTF8 qw(ord);" exists or not. If $expr is omitted, it uses $_.
  The return value is always unsigned.
...
=item * Reverse List or Byte String
  ...
  "use OldUTF8 qw(reverse);" exists or not.
...
=head1 Un-Escaping bytes::* Subroutines (OldUTF8 software provides)

OldUTF8 software removes 'bytes::' at head of subroutine name.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Eoldutf8.pm provides these
  functions.
...
some bugs. Thus, if you happen to find a bug that's in OldUTF8 software and
...
=item * OldUTF8::substr as Lvalue

If Perl version is older than 5.14, OldUTF8::substr differs from CORE::substr, and
...
OldUTF8::substr($string, 13, 4, "JPerl");
...
OldUTF8 software remains one language and one interpreter.
