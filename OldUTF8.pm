...
sub e_capture {
    ...
    return join '', '${Eoldutf8::capture(', $_[0], ')}';
    return join '', '$',                 $_[0];
}
...
impossible. OldUTF8 software is Perl program designed to make the "easy jobs easy".
...
=head1 Multiple-Octet Anchoring of Regular Expression (OldUTF8.pm provides)

OldUTF8.pm applies multiple-octet anchoring at beginning of regular expression.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/regexp/               m/${Eoldutf8::anchor}(?:regexp).../
  --------------------------------------------------------------------------------
...
functions.
  ...
  index       Eoldutf8::index     Character
  rindex      Eoldutf8::rindex    Character
...
=item * Special Variables $` and $& need /( Capture All )/
  ...
  Before          After                Works as
  -------------------------------------------------------------------------------------------
  $`              Eoldutf8::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $PREMATCH       Eoldutf8::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  ${^PREMATCH}    Eoldutf8::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $&              Eoldutf8::MATCH()       $1
  $MATCH          Eoldutf8::MATCH()       $1
  ${^MATCH}       Eoldutf8::MATCH()       $1
  $'              Eoldutf8::POSTMATCH()   $'
  $POSTMATCH      Eoldutf8::POSTMATCH()   $'
  ${^POSTMATCH}   Eoldutf8::POSTMATCH()   $'
  -------------------------------------------------------------------------------------------
