...
my $qq_substr  = qr{(?: OldUTF8::substr | CORE::substr | substr ) \( $qq_paren \)
                 }xms;
...
my $function_ord;         # ord()   to ord()   or OldUTF8::ord()
my $function_ord_;        # ord     to ord     or OldUTF8::ord_
my $function_reverse;     # reverse to reverse or OldUTF8::reverse
my $function_getc;        # getc    to getc    or OldUTF8::getc
...
END
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'OldUTF8::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'OldUTF8::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'OldUTF8::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'OldUTF8::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $OldUTF8::re_t=%s; while($OldUTF8::re_t =~ %s){%s local $^W=0; local $OldUTF8::re_r=%s; %s$OldUTF8::re_t="$`$OldUTF8::re_r$'"; pos($OldUTF8::re_t)=length "$`$OldUTF8::re_r"; } return $OldUTF8::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$OldUTF8::re_r=eval $OldUTF8::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $OldUTF8::re_r=%s; %s"%s$OldUTF8::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $OldUTF8::re_n=0; while(%s =~ %s){%s local $^W=0; local $OldUTF8::re_r=%s; %s%s="$`$OldUTF8::re_r$'"; pos(%s)=length "$`$OldUTF8::re_r"; $OldUTF8::re_n++} return %s$OldUTF8::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $OldUTF8::re_r=%s; %s%s="%s$OldUTF8::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $OldUTF8::re_r=%s; %s%s="%s$OldUTF8::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$OldUTF8::re_r=eval $OldUTF8::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
expand wildcard arguments supplied to programs. Instead, import of Eoldutf8.pm
works well.
...
=head1 Multiple-Octet Character Regular Expression (OldUTF8.pm provides)

OldUTF8.pm clusters multiple-octet character with quantifier, makes cluster from
...
=head1 Calling 'Eoldutf8::ignorecase()' (OldUTF8.pm provides)

OldUTF8.pm applies calling 'Eoldutf8::ignorecase()' instead of /i modifier.
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  s/regexp/replacement/   eval{ Eoldutf8::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
 
  $_ !~                   ($_ !~ m/ditto$Eoldutf8::matched/) ?
  s/regexp/replacement/   1 :
                          eval{ Eoldutf8::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; undef }
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  s/regexp/replacement/b  eval{ Eoldutf8::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
...
=head1 Un-Escaping \ of \N, \p, \P and \X (OldUTF8.pm provides)

OldUTF8.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (OldUTF8.pm and Eoldutf8.pm provide)
...
=item * Returns Next Character
  ...
  If you import getc "use OldUTF8 qw(getc);", getc of your script will be rewritten in
  OldUTF8::getc. OldUTF8::getc is not compatible with getc of JPerl.
...
=item * Length by old UTF-8 Character
  ...
  the scalar value $string. If $string is omitted, it returns the OldUTF8::length of
  $_.
...
=item * Substr by old UTF-8 Character
  ...
  If Perl version 5.14 or later, you can use the OldUTF8::substr() function as an
...
=item * Rindex by old UTF-8 Character
  ...
  This function works just like OldUTF8::index except that it returns the character
...
=item * Filename Globbing
  ...
  This operator is implemented via the Eoldutf8::glob() function. See Eoldutf8::glob of
  Eoldutf8.pm for details.
...
=head1 Un-Escaping bytes::* Functions (OldUTF8.pm provides)

OldUTF8.pm removes 'bytes::' at head of function name.
...
=head1 Escaping Built-in Standard Module (Eoldutf8.pm provides)

Eoldutf8.pm does "BEGIN { unshift @INC, '/Perl/site/lib/OldUTF8' }" at head.
Store the standard module modified for OldUTF8 software in this directory to
override built-in standard modules.

=head1 Escaping Standard Module Content (You do)

You need copy built-in standard module to /Perl/site/lib/OldUTF8 and change
'use utf8;' to 'use OldUTF8;' in its. You need help yourself for now.
...
OldUTF8::substr differs from CORE::substr, and cannot be used as a lvalue.
