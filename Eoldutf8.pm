...
# @{Eoldutf8::eS}        = qr{(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x00-\xFF]|[^\x80-\xFF\x09\x0A\x0B\x0C\x0D\x20])};
...
if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
    if ($ENV{'ComSpec'} =~ / (?: COMMAND\.COM | CMD\.EXE ) \z /oxmsi) {
        ...
        for (@ARGV) {
            ...
            elsif (/\A (?:$q_char)*? [*?] /oxms and (my @glob = Eoldutf8::glob($_))) {
                push @argv, @glob;
            }
            else {
                push @argv, $_;
...
#
{
    ...
    sub Eoldutf8::matched() {
        $last_s_matched = 0;
    }

    # old UTF-8 mark last s/// matched
    sub Eoldutf8::s_matched() {
        $last_s_matched = 1;
    }
    ...
    @Eoldutf8::matched = (qr/(?{Eoldutf8::matched})/);
}
