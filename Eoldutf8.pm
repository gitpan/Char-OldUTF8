...
=item Split string
  ...
  $string on every match. If the pattern doesn't match at all, Eoldutf8::split returns
...
=item Chop string
  ...
  The Eoldutf8::chop operator is used primarily to remove the newline from the end of
  an input record but is more efficient than s/\n$//. If no argument is given, the
  function chops the $_ variable.

=item Index string

  $pos = Eoldutf8::index($string,$substr,$position);
  $pos = Eoldutf8::index($string,$substr);
  ...
  the Eoldutf8::index function returns -1.

=item Reverse index string

  $pos = Eoldutf8::rindex($string,$substr,$position);
  $pos = Eoldutf8::rindex($string,$substr);

  Works just like Eoldutf8::index except that it returns the position of the last
...
=item Make character
  ...
  set. For example, Eoldutf8::chr(65) is "A" in either ASCII or old UTF-8, not Unicode,
  and Eoldutf8::chr(0x82a0) is a old UTF-8 HIRAGANA LETTER A. For the reverse of
  Eoldutf8::chr, use OldUTF8::ord.
