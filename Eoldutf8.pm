...
BEGIN {
    ...
    if ($] < 5.006) {
        ...
        if ($^O ne 'MacOS') {
            *CORE::GLOBAL::binmode =
            *CORE::GLOBAL::binmode = \&Eoldutf8::binmode;
        }
        *CORE::GLOBAL::open    =
        *CORE::GLOBAL::open    = \&Eoldutf8::open;
    }
}
...
sub charlist_not_qr {
    ...
    if (scalar(@charlist) >= 1) {
        if (scalar(@singleoctet) >= 1) {
            ...
            return '(?!' . join('|', @charlist) . ')(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x00-\xFF]|[^'. join('', @singleoctet) . '])';
        }
        else {
        ...
        if (scalar(@singleoctet) >= 1) {
            ...
            return                                 '(?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x00-\xFF]|[^'. join('', @singleoctet) . '])';
        }
        else {
...
sub _dosglob {
    ...
    # UNIX-like system
        ...
        $expr =~ s{ \A ~ ( (?:(?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x00-\xFF]|[^/])* ) }
...
OUTER:
    for my $expr (@expr) {
        ...
        if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
            $expr =~ s# \A ((?:[A-Za-z]:)?) ((?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x00-\xFF]|[^/\\]) #$1./$2#oxms;
        }
...
sub _parse_line {
    ...
    my @piece = ();
    while ($line =~ m{
        " ( (?: (?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x00-\xFF]|[^"]   )*  ) " \s+ |
          ( (?: (?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x00-\xFF]|[^"\s] )*  )   \s+
        }oxmsg
    ) {
...
sub _parse_path {
    ...
    my @subpath = ();
    while ($path =~ m{
        ((?: (?:[\xC0-\xDF]|[\xE0-\xEF][\x80-\xBF]|[\xF0-\xF4][\x80-\xBF][\x80-\xBF])[\x00-\xFF]|[^/\\] )+?) [/\\] }oxmsg
    ) {
        push @subpath, $1;
    }
...
=item Make character
  ...
  set. For example, Eoldutf8::chr(65) is "A" in either ASCII or old UTF-8, and
  Eoldutf8::chr(0x82a0) is a old UTF-8 HIRAGANA LETTER A. For the reverse of Eoldutf8::chr,
  use OldUTF8::ord.
...
=item binary mode (Perl5.6 emulation on perl5.005)
  ...
  Eoldutf8::binmode(FILEHANDLE, $disciplines);
  Eoldutf8::binmode(FILEHANDLE);
  Eoldutf8::binmode($filehandle, $disciplines);
  Eoldutf8::binmode($filehandle);
...
=item open file (Perl5.6 emulation on perl5.005)
  ...
  $rc = Eoldutf8::open(FILEHANDLE, $mode, $expr);
  $rc = Eoldutf8::open(FILEHANDLE, $expr);
  $rc = Eoldutf8::open(FILEHANDLE);
  $rc = Eoldutf8::open(my $filehandle, $mode, $expr);
  $rc = Eoldutf8::open(my $filehandle, $expr);
  $rc = Eoldutf8::open(my $filehandle);
  ...
  * three arguments
  ...
  {
      my $fh;                          # (uninitialized)
      Eoldutf8::open($fh, ">logfile")     # $fh is autovivified
  ...
  Eoldutf8::open my $fh, ">logfile" or die ...
  ...
  Eoldutf8::open(LOG, ">", "logfile") or die "Can't create logfile: $!";
  ...
  $path =~ s#^([ ])#./$1#;
  Eoldutf8::open (FH, "< $path\0") or die "can't open $path: $!";
  ...
  Eoldutf8::open(FH, "<", $path) or die "can't open $path: $!";
  ...
  Eoldutf8::open(FH, "<:raw", $path) or die "can't open $path: $!";
  ...
  Eoldutf8::open(FH, "<:crlf:encoding(OldUTF8)", $path) or die "can't open $path: $!";
